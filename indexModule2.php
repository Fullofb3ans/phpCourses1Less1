...
<div class="col-md-3 col-sm-3 ">
 ...
 echo '<div>';
    ...
    {
      echo "<div>$countPublishers</div>", '<br>';
   }
   else {
    echo '<div>Нет элементов</div>', '<br>';
   }
...
$number = 10;
  ...
  switch ($word) {
    case 'index':
    echo "<h2>Главная</h2>", '<hr/>';
    break;
    
    case 'basket':
    echo "<h2>Корзина</h2>", '<hr/>';
      break;    
      case 'contacts':
    echo "<h2>Контакты</h2>", '<hr/>';
      break;    
      case 'delivery':
    echo "<h2>Доставка</h2>", '<hr/>';
      break;    
      case 'login':
    echo "<h2>Вход</h2>", '<hr/>';
      break;
  ...
  // №3
  if($number>0){
    echo "Осталось $number Товаров", '<hr/>';
  }
  // Практическая 2.2 № 1
  $categories = ['Action', 'Fantastic', 'Horror', 'Romantic'];
  $publisher = ['Бука', 'Новая книга'];
    ...
    for ($i=0; $i < count($books); $i++) { 
      ...
      <div class='card'>        
        ...
        <div class='card-footer'>
          <button type='button' class='btn btn-primary'><a href='?add={$books[$i]['idbook']}'>В корзину</a></button>
        </div>
       </div>";
    }
